...
def check():
    ...
    for name in os.listdir(path):
        if not os.path.exists(os.path.join(path2, name)):
            if os.path.isdir(os.path.join(path, name)):
                logger.warn('Found directory to add: ' +
                            name, 'ResourceLoader')
    ...
    for name in os.listdir(path2):
        if not os.path.exists(os.path.join(path, name)):
            if os.path.isdir(os.path.join(path2, name)):
                logger.warn('Found directory to remove: ' +
                            name, 'ResourceLoader')
                count2 += 1
    ...
    logger.info('Added ' + str(count) +
                ' files/directories.', 'ResourceLoader')
    logger.info('Removed ' + str(count2) +
                ' files/directories.', 'ResourceLoader')
