...
def dif(difficulty):
    if difficulty == 'Default':
        return 1
    elif difficulty == 'Adventure':
        return 2
    elif difficulty == 'Apocalypse':
        return 3
    else:
        logger.error(f'Difficulty {difficulty} does not exist.')
        return None
...
class Game:
    def __init__(self, spx, spy, using_save, difficulty, name='New World', filename='New World'): #the game object that is created when a world is loaded
        #chest loot pool
        ...
        self.player = dungeon_player.Player(spx, spy, dif(difficulty)) #make the player
    ...
    def update_while_chat(self, events, text, pos): #function that runs while chat is being entered
        ...
        if self.enemy_delay > 500 + random.randint(-20, 20) - (self.player.difficulty * 7 * self.dif):
            self.add_enemy()
            self.enemy_delay = 0

        self.chest_delay += 1 #spawn chests
        if self.chest_delay > 600 + random.randint(-20, 20) - ((self.player.difficulty * 7 * self.dif) // 2):
            self.spawn_chest()
            self.chest_delay = 0
...
def show_credits():
    ...
    text = [pygame.font.Font(PATH, 70).render('Coded by: VolcanicAsh999', 1, pygame.Color('black')),
            pygame.font.Font(PATH, 70).render('Characters created by: VolcanicAsh999', 1, pygame.Color('black')),
            pygame.font.Font(PATH, 70).render('Pretty much everything: VolcanicAsh999', 1, pygame.Color('black')),
...
def play(filename):
    ...
    pygame.display.update()
    game = Game(spx, spy, dungeon_settings.RSAVE, difficulty, saving.worldname(filename), filename) #make a game
    saving = game.saving
...
def new_world():
    ...
    game = Game(spx, spy, False, difficulty, name, name)
...
def main(options=None):
    ...
    button_difficulty = dungeons_gui.Button(pygame.Rect((screen.get_width() - 350), 10, 300, 80), 'light gray', 'Default', change_difficulty, 60, PATH)
    ...
    try:
        while True:
            ...
            if button_difficulty.text != difficulty:
                button_difficulty.set_text(difficulty)
            for event in events:
                if event.type == pygame.QUIT:
                    logger.info('Window has closed.')
                    logger.info('Stopping!')
                    ...
                    button_difficulty.update_((screen.get_width()  - 350), 10, 300, 80, 60)
