...
def gettextinput(pos: tuple[int], screen: pygame.Surface, color: pygame.Color = pygame.Color('black'), accepttext: str = string.printable, fontname: str = 'default', fontsize: int = 20, whilerunfunc: tuple = ('function', 'args'), other_text: str = '> ', fillcolor: pygame.Color = None, doupdate=True, quitonesc=True, passtext=False, passevents=True):
    if os.path.exists(os.path.join(os.getcwd(), fontname)):
        font_obj = pygame.font.Font(os.path.join(os.getcwd(), fontname), fontsize)
    elif os.path.exists(fontname):
        font_obj = pygame.font.Font(fontname, fontsize)
    elif fontname != 'default':
        try: font_obj = pygame.font.SysFont(fontname, fontsize)
        except: font_obj = pygame.font.SysFont('Sans Mono', fontsize)
    else:
        font_obj = pygame.font.SysFont('Sans Mono', fontsize)
    run = True
    if (whilerunfunc == ('function', 'args')) or (type(whilerunfunc[0]) not in [types.FunctionType, types.BuiltinFunctionType, types.MethodType, types.BuiltinMethodType]) or (type(whilerunfunc[1]) != tuple):
        whilerunfunc = (lambda: ..., ())
    text = ''
    while run:
        events = pygame.event.get()
        if fillcolor != None: screen.fill(fillcolor)
        for event in events:
        ...
        if passtext:
            if passevents:
                whilerunfunc[0](*whilerunfunc[1], events, font_obj.render(other_text + text, 1, color), pos)
            else:
                whilerunfunc[0](*whilerunfunc[1], font_obj.render(other_text + text, 1, color), pos)
        else:
            if passevents:
                whilefunc[0](*whilerunfunc[1], events)
                screen.blit(font_obj.render(other_text + text, 1, color), pos)
            else:
                whilefunc[0](*whilerunfunc[1])
                screen.blit(font_obj.render(other_text + text, 1, color), pos)
        if doupdate: pygame.display.update()
